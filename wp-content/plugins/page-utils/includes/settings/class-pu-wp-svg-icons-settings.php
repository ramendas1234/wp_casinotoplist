...
/**
 * PU_WP_SVG_Icons_Admin_Settings class.
 */
class PU_WP_SVG_Icons_Admin_Settings {
	...
	public function __construct() {
		$this->options = get_option( 'wp_svg_icons_settings_options' );
		add_action( 'admin_init', array( $this, 'settings_page_init' ) );
        add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_wp_svg_scripts' ) );
	}
    ...
    public function settings_page_init() {        
        register_setting(
            'wp_svg_icons_settings_option_group', // Option group
            'wp_svg_icons_settings_options', // Option name
            array( $this, 'sanitize_settings' ) // Sanitize
        );

        add_settings_section(
            'wp_svg_icons_general_settings', // ID
            __( 'General', 'page-utils' ), // Title
            array( $this, 'wp_svg_icons_general_settings_section_info' ), // Callback
            'wp_svg_icons-settings-admin' // Page
        );  

        add_settings_field(
            'default_icons_pack', // ID
            __( 'Default Icons Set', 'page-utils' ), // Title 
            array( $this, 'default_icons_pack_field_callback' ), // Callback
            'wp_svg_icons-settings-admin', // Page
            'wp_svg_icons_general_settings' // Section           
        );

        add_settings_field(
            'defualt_icon_wrapper', // ID
            __( 'Set Default Icons wrapper', 'page-utils' ), // Title 
            array( $this, 'defualt_icon_wrapper_field_callback' ), // Callback
            'wp_svg_icons-settings-admin', // Page
            'wp_svg_icons_general_settings' // Section           
        );
    ...
    public function enqueue_wp_svg_scripts() {
        wp_enqueue_style( 'page_utils_css', PU()->plugin_url() . '/assets/css/page_utils.css', array(), PU_VERSION );
        wp_register_script( 'wp-svg-icon-scripts', PU()->plugin_url() . '/assets/js/wp-svg-icon-plugin-scripts.js', array( 'jquery' , 'jquery-ui-core' , 'jquery-ui-slider' ), PU_VERSION, true );
        $localized_data = array(
            'site_url' => site_url(),
            'custom_pack_active' => false,
            'default_icon_element' => isset($this->options['defualt_icon_wrapper']) ? $this->options['defualt_icon_wrapper'] : 'i',
        );
        wp_localize_script( 'wp-svg-icon-scripts' , 'localized_data' , $localized_data );
        wp_enqueue_script( 'wp-svg-icon-scripts' );
    }
    ...
    public function sanitize_settings( $input ) {
        ...
        if( isset( $input['defualt_icon_wrapper'] ) )
            $new_input['defualt_icon_wrapper'] = $input['defualt_icon_wrapper'];
     ...
     */
    public function wp_svg_icons_general_settings_section_info() {
     ...
     * Get settings fields
     */
    public function defualt_icon_wrapper_field_callback() {
        $default_icon_container = isset($this->options['defualt_icon_wrapper']) ? $this->options['defualt_icon_wrapper'] : 'i';
        ?>
        <select id="defualt_icon_wrapper" name="wp_svg_icons_settings_options[defualt_icon_wrapper]">
            <option value="i" <?php selected( $default_icon_container , 'i' ); ?>>i</option>
            <option value="H1" <?php selected( $default_icon_container , 'h1' ); ?>>h1</option>
            <option value="h2" <?php selected( $default_icon_container , 'h2' ); ?>>h2</option>
            <option value="h3" <?php selected( $default_icon_container , 'h3' ); ?>>h3</option>
            <option value="span" <?php selected( $default_icon_container , 'span' ); ?>>span</option>
            <option value="div" <?php selected( $default_icon_container , 'div' ); ?>>div</option>
            <option value="b" <?php selected( $default_icon_container , 'b' ); ?>>b</option>
        </select>
        <?php
    }
     ...
     * Get settings fields
     */
    public function default_icons_pack_field_callback() {
        ?>
        <div class="wp_svg_icons_default_icons_pack_container" style="width: 100%;
    ...
    padding-bottom: 20px;">
            <div class="how-to-use">
                ...
                <?php $selected = isset($this->options['defualt_icon_wrapper']) ? $this->options['defualt_icon_wrapper'] : 'i'; ?>
            ...
            <div class="wp-svg-iconset1-all-glyphs" style="display:inline-block; margin-top:1em; border-top: 1px solid #ccc; ">
...
return new PU_WP_SVG_Icons_Admin_Settings();